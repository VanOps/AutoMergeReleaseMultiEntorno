...
if [ $ISSUES_FOUND -eq 0 ] && [ $WARNINGS -eq 0 ]; then
  ...
  echo "   7. Back-merge PR created automatically to develop"
  echo "   8. Approve back-merge PR (if develop is protected)"
...
echo ""
echo "📖 Pipeline Flow:"
echo "   develop → release/vX.X → QA → Staging (approval) → PR to main → Production (approval) → back-merge PR to develop (approval if protected)"
echo ""
